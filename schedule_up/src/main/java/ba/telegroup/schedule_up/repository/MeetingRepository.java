...
public interface MeetingRepository extends JpaRepository<Meeting,Integer>{
    ...
    List<Meeting> getAllByStatusInParticipant(Integer participantId);
    ...
    @Query(value = "SELECT r.id, r.topic, r.start_time,r.end_time,r.participants_number,r.status,r.description, r.cancelation_reason, " +
            "r.room_id,r.user_id,r.company_id FROM meeting r JOIN Participant p ON r.id=p.meeting_id WHERE (status=0 or status=1) and p.deleted=0 " +
